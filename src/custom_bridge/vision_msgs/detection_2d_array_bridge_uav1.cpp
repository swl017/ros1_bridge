...
void ros1Detection2DArrayCallback(const ros::MessageEvent<vision_msgs::Detection2DArray const> & ros1_msg_event)
{
  ...
  if (connection_header->find(key) != connection_header->end()) {
    if (connection_header->at(key) == "/uav1_detection_2d_array_bridge") {
...
int main(int argc, char * argv[])
{
  ...
  ros::init(argc, argv, "uav1_detection_2d_array_bridge");
  ros::NodeHandle ros1_node;
  ros1_pub = ros1_node.advertise<vision_msgs::Detection2DArray>("/uav1/yolo_result_vision", 10);

  // ROS 2 node and publisher
  rclcpp::init(argc, argv);
  auto ros2_node = rclcpp::Node::make_shared("uav1_detection_2d_array_bridge");
  ros2_pub = ros2_node->create_publisher<vision_msgs::msg::Detection2DArray>(
    "/uav1/yolo_result_vision", 10);

  // ROS 1 subscriber
  ros::Subscriber ros1_sub = ros1_node.subscribe(
    "/uav1/yolo_result_vision", 10, ros1Detection2DArrayCallback);

  // ROS 2 subscriber
  rclcpp::SubscriptionOptions options;
  options.ignore_local_publications = true;
  auto ros2_sub = ros2_node->create_subscription<vision_msgs::msg::Detection2DArray>(
    "/uav1/yolo_result_vision", rclcpp::SensorDataQoS(), ros2Detection2DArrayCallback, options);
